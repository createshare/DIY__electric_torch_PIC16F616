...
unsigned int  LongKeyCnt = 0;  //长按计数器.//时基为10ms ("循环"执行一次程序)
...
//pKeyResult = G_InPut165_01;     //按键值(从74HC165的第一个字节采样)更新
	...
	else
	{
		...
		if(pKeyNullCnt >= M_KeyDelayTime)
		{
			...
			KeyBuffer = M_NullKey;  //按键弹起消抖后，为无效按键
	...
	if(pKeyResult == 0x00) KeyBuffer = 1;       //按键1
	//else if(pKeyResult == 0x04) KeyBuffer = 2;  //按键2

	//if(pKeyResult == 0x0E) KeyBuffer = 1;       //按键1
	//else if(pKeyResult == 0x0D) KeyBuffer = 2;  //按键2
	//else if(pKeyResult == 0x0B) KeyBuffer = 3;  //按键3
	//else if(pKeyResult == 0X07) KeyBuffer = 4;  //按键4
	//else if(pKeyResult == 0X2F) KeyBuffer = 5;  //按键5
	//else if(pKeyResult == 0X1F) KeyBuffer = 6;  //按键6
	/////else if(pKeyResult == 0XBF) KeyBuffer = 7;  //按键7
	/////else if(pKeyResult == 0X7F) KeyBuffer = 8;  //按键8
	//else if((F_Sw7_165==0)&&(F_Sw8_165==0)) KeyBuffer = 9;  //按键9 //7键和8键同时按下时的按键值定义为9

		 //其他情况的按键，作为无效按键//good,当用到两个按键同时按下才有效的情况,一定要加入下面的程序else rKeyBuffer = M_NullKey; 
	else KeyBuffer = M_NullKey; 


	if(F_KeyHave)
	{
		if(KeyBuffer != pKeyBuffer1)
		{
			pKeyBuffer1 = KeyBuffer;
			pKeyCnt = 0;
		}
		else
		{
...
{     
	...
	{
		LongKeyCnt++;  //时基为10ms
		if(LongKeyCnt > SecondNum*100)  //长按时间为pNum/100秒
		{
			LongKeyCnt = 0; //清长按计数器.
